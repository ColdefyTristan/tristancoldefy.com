...
from app.models.riftdle.schemas import ClueType
from app.utils.time import utcnow_naive, today_paris_date


class RiftdleAttempt(SQLModel, table=True):
    __tablename__ = "riftdle_attempt"
    __table_args__ = (
        UniqueConstraint("user_id", "day", name="uq_riftdle_attempt_user_day"),
        Index("ix_riftdle_attempt_user_day", "user_id", "day"),
        Index("ix_riftdle_attempt_day", "day"),
    )
    ...
    guesses: list["RiftdleAttemptGuess"] = Relationship(back_populates="attempt")
    user: "User" = Relationship()


class RiftdleAttemptGuess(SQLModel, table=True):
    __tablename__ = "riftdle_attempt_guess"
    __table_args__ = (
    ...
    attempt_id: int = Field(
        sa_column=Column(
            Integer,
            ForeignKey("riftdle_attempt.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        )
    )
    ...
    attempt: RiftdleAttempt = Relationship(back_populates="guesses")
