...
def test_anonymous_guess_returns_result_without_db_write(session, client, champ_factory):
    champ_factory(name="Lux", is_champ_of_the_day=False)
    champ_factory(name="Jinx", is_champ_of_the_day=True)
...
def test_anonymous_correct_guess_returns_is_correct_true(client, champ_factory):
    champ_factory(name="Jinx", is_champ_of_the_day=True)
...
def test_authenticated_guess_creates_attempt_in_db(session, auth_client, champ_factory):
    champ_factory(name="Lux", is_champ_of_the_day=False)
    champ_factory(name="Jinx", is_champ_of_the_day=True)
    ctx = auth_client()
...
def test_authenticated_correct_guess_finishes_attempt(auth_client, champ_factory):
    champ_factory(name="Jinx", is_champ_of_the_day=True)
    ctx = auth_client()
...
def test_guess_after_finished_attempt_returns_409(auth_client, champ_factory):
    champ_factory(name="Jinx", is_champ_of_the_day=True)
    ctx = auth_client()
    authed = ctx["client"]
...
def test_get_today_attempt_returns_attempt_after_guess(auth_client, champ_factory):
    champ_factory(name="Lux", is_champ_of_the_day=False)
    champ_factory(name="Jinx", is_champ_of_the_day=True)
    ctx = auth_client()
    authed = ctx["client"]
